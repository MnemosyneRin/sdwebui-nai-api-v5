...
class NAIGENScriptBase(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        with gr.Accordion(label=self.NAISCRIPTNAME, open=False):
            ...
            with gr.Accordion(label="Options", open=False):
                ...
                with gr.Row():
                    enhance = gr.Dropdown(value="Off", label="Enhance/Upscale (COSTS ANLAS)", choices=nai_api.enhance_scales, type="value")
                    enhance_magnitude = gr.Slider(minimum=1, maximum=5, step=1, value=3, label="Enhance Magnitude")
        ...
        self.infotext_fields = [
            ...
            (enhance, f'{PREFIX} '+ 'enhance'),
            (enhance_magnitude, f'{PREFIX} '+ 'enhance_magnitude'),
    ...
    def nai_generate_images(self,p,enable,convert_prompts,cost_limiter,nai_post,disable_smea_in_post,model,sampler,noise_schedule,dynamic_thresholding,variety,smea,cfg_rescale,skip_cfg_above_sigma,qualityToggle,ucPreset,fur_dataset,do_local_img2img,extra_noise,inpaint_mode,nai_resolution_scale,nai_cfg,nai_steps,nai_denoise_strength,legacy_v3_extend,augment_mode,defry,emotion,reclrLvlLo,reclrLvlHi,reclrLvlMid,reclrLvlLoOut,reclrLvlHiOut,reclrLvlAlpha,deliberate_euler_ancestral_bug,prefer_brownian,legacy_uc,normalize_reference_strength_multiple,normalize_negatives,normalize_level,cref_image,cref_style,cref_fidel,transparent,enhance,enhance_magnitude,keep_mask_for_local,*args):
        ...
        if self.augment_mode:
            ...
            if enhance and enhance != "Off":
                ...
                p.extra_generation_params[f'{PREFIX} '+ 'enhance_magnitude'] = enhance_magnitude
        ...
        if enhance and enhance != "Off" and not self.augment_mode:
            self.nai_enhance_images(p, enhance, enhance_magnitude, getparams)
    ...
    def nai_enhance_images(self, p, enhance, magnitude, getparams):
        """Second NAI pass at a larger size, mirroring the site's Enhance button. Always costs Anlas."""
        key = self.get_api_key()
        self.comment(p, f"Enhance {enhance} (magnitude {magnitude}): this pass is NOT covered by the Opus free limit and spends Anlas.")
        for i in range(len(self.images)):
            ...
            if w * h > nai_api.MAX_ENHANCE_PIXELS:
                ...
                continue
            DEBUG_LOG(f"Enhance {i}: {image.width}x{image.height} -> {w}x{h}")
            self.begin_request(p, i, params)
            result = nai_api.POST(key, params, timeout = nai_api.get_timeout(shared.opts.nai_api_timeout, w, h, p.steps), attempts = shared.opts.nai_api_retry, wait_on_429 = shared.opts.nai_api_wait_on_429, wait_on_429_time = shared.opts.nai_api_wait_on_429_time)
            if result.status_code != 200 or not result.files:
                self.comment(p, f"Enhance failed for image {i+1}: {result.message}")
                continue
            enhanced = result.files[0]
            enhanced.original_nai_image = image
            self.images[i] = enhanced
            self.last_request_time = time.time()
    ...
    def load_image(self, p, i, parameters, batch_size = 1):    
        ...
        if shared.opts.live_previews_enable and not self.augment_mode and self.isV4 and shared.opts.nai_api_preview:
            def preview(img, step):            
                ...
                if step < shared.state.current_image_sampling_step: shared.state.current_image_sampling_step = 0 # NAI restarted the generation
