...
def on_ui_settings():
    ...
    addopt('nai_api_save_during_batch', shared.OptionInfo(True, "Save Images During Batch Processing. For compatibility, all images must be generated before running Post-Processing, and normally images are not saved until all batches are completed. If set, Images will be saved during Batch Processing, before Post-Processing. If Post-processing scripts modify the result images, images will be saved again after batch is completed.",gr.Checkbox, section=section))
